...
public interface BookingRepository extends JpaRepository<Booking, Integer> {
    ...
    List<Booking> findPastByBooker(User booker, LocalDateTime now);

    @Query(value = "select b from Booking b " +
            "where b.booker = :booker and b.start > :now order by b.start desc ")
    List<Booking> findFutureByBooker(User booker, LocalDateTime now);
    ...
    List<Booking> findCurrentByOwner(User owner, LocalDateTime now);

    @Query(value = "select b from Booking b " +
            "where b.item.owner = :owner and b.status = :status and b.start > :now order by b.start desc ")
    List<Booking> findAllByOwnerAndStatusOrderByStartDesc(User owner, State status);

    @Query(value = "select b from Booking b " +
            "where b.item.owner = :owner and b.end < :now order by b.start desc ")
    List<Booking> findPastByOwner(User owner, LocalDateTime now);

    @Query(value = "select b from Booking b " +
            "where b.item.owner = :owner and b.start > :now order by b.start desc ")
    List<Booking> findFutureByOwner(User owner, LocalDateTime now);
}
